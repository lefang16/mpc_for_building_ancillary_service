function options = settings_asOpt(varargin)
...
%options.NLPsolver='OSQP';

% OSQP settings (if required)
options.OSQP.alpha=1.6;
options.OSQP.rho=0.1;
options.OSQP.sigma=1e-9;
options.OSQP.max_iter=100000;
options.OSQP.eps_abs=1e-09;
options.OSQP.eps_prim_inf=1e-09;
options.OSQP.eps_dual_inf=1e-09;
options.OSQP.eps_rel=1e-09;


% IPOPT settings (if required)
%---------------------------------------
options.ipopt.tol=1e-12;                        % Desired convergence tolerance (relative). The default value is  1e-8.
...
% options.ipopt.constr_viol_tol=0.00001;           % Absolute tolerance on the constraint violation.
%                                                % Successful termination requires that the max-norm of the (unscaled) constraint violation is less than this threshold.
%                                                % Default value 0.0001
%
% options.ipopt.compl_inf_tol=0.00001;               %Absolute tolerance on the complementarity.  Default value is 0.0001
%
% options.ipopt.acceptable_tol=10-07;           %Determines which (scaled) overall optimality error is considered to be "acceptable".
%                                                  % Default value is 10-06
% options.ipopt.acceptable_constr_viol_tol=0.0001;   % Absolute tolerance on the constraint violation. Default value is 0.01.
...
% Skip the post-solve analysis after the solution of the problem
options.skipPostAnalysis=0;
